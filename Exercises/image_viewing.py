# this file contains the ugly code hacked together my overworked PhD students to hide boring visualisation boilerplate code from users and keep the coarse interesting
...
def overlay_RGB(I_list):
    ...
    for i in range(len(I_list)-1):
        ...
        overlay[:,:,2] = I_list[i].astype(float)*0.5 + I_list[i+1].astype(float)*0.5
        if (len(I_list)-1) == 1:
...
def overlay_RGB_cost(I_list,cost_history,cost_function,cfuncs,x,y, history=None):
    ...
    for i in range(len(I_list)-1):
        ...
        if isinstance(cost_fun,dict):
            ...
            for name,f in cost_fun.items():
                ...
                if history is None:
                    for i in range(len(ch)-1):
                        ax.plot(ch[i:i+2,0], ch[i:i+2,1], ch[i:i+2,2], color=plt.cm.jet(0.5*(colour[i]+colour[i+1])))
                else:
            ...
            cost_history.append((x,y,cost_fun(I_list[0],I_list[1]),numpy.prod(I_list[0].shape )))
        ...
        if len(cost_history)>1:
            t1+="\n Gradient: " +"{0:.3f}".format((cf-cost_history[cost_function][-2][2]))
        ax.set_title(t1)
